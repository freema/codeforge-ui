...
const KEY_PROVIDERS = [
  { value: "github", label: "GitHub", icon: "code", description: "GitHub personal access token" },
  { value: "gitlab", label: "GitLab", icon: "cloud", description: "GitLab personal access token" },
  { value: "sentry", label: "Sentry", icon: "bug_report", description: "Sentry authentication token" },
] as const;
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {k.base_url && (
                          <span className="font-mono text-[10px] text-fg-4">{k.base_url}</span>
                        )}
                        {k.scope && (
        ...
        <div className="rounded-xl border border-edge bg-surface/50 p-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {KEY_PROVIDERS.map((p) => (
              ...
              >
                <span className="material-symbols-outlined text-xl text-fg-3 group-hover:text-accent">{p.icon}</span>
                <div>
          ...
          <div className="mb-4 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-fg-2">
              <span className="material-symbols-outlined text-accent text-base">
                {KEY_PROVIDERS.find((p) => p.value === provider)?.icon ?? "vpn_key"}
              </span>
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              ...
              {headerCount > 0 && (
                <span className="flex items-center gap-1 text-[10px] text-fg-4">
                  <span className="material-symbols-outlined text-xs">http</span>
              ...
              {server.created_at && (
                <span className="text-[10px] text-fg-4">{relativeTime(server.created_at)}</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="shrink-0">
          {confirmDelete ? (
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setConfirmDelete(false)} className="text-xs text-fg-3">
                Cancel
              </button>
            </div>
          ) : (
...
function KeyValueEditor({
  ...
  function updateRow(index: number, field: 0 | 1, value: string) {
    const next = entries.map((e, i) => (i === index ? (field === 0 ? [value, e[1]] : [e[0], value]) as [string, string] : e));
    onChange(next);
  }
...
function MCPTab() {
  ...
  const [formMode, setFormMode] = useState<"select" | "preset" | "custom">("select");
  ...
  async function handleCustomSubmit(e: FormEvent) {
    ...
    if (!NAME_PATTERN.test(name)) {
      setValidationError("Name must contain only letters, numbers, hyphens, and underscores");
      return;
    }
    ...
    if (args.trim()) {
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          parsedArgs = args.split(",").map((a) => a.trim()).filter(Boolean);
        }
      } catch {
        parsedArgs = args.split(",").map((a) => a.trim()).filter(Boolean);
      }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-xl border border-edge bg-surface/50 p-5">
          ...
          {catalogLoading ? (
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                ...
                return (
                  ...
                  >
                    <div className="flex w-full items-center justify-between">
                      <span className={`material-symbols-outlined text-xl ${alreadyAdded ? "text-accent" : "text-fg-3 group-hover:text-accent"}`}>
                    ...
                    <div>
                      <p className="text-sm font-medium text-fg capitalize">{tool.name}</p>
                      <p className="mt-0.5 text-xs text-fg-4 line-clamp-2">{tool.description}</p>
                    </div>
                    {(tool.required_config?.length ?? 0) > 0 && (
                      <span className="text-[10px] text-fg-4">
                        {tool.required_config!.length} config field{tool.required_config!.length !== 1 && "s"}
                      </span>
                    )}
                  </button>
                );
              ...
              >
                <span className="material-symbols-outlined text-xl text-fg-3 group-hover:text-accent">dns</span>
                <div>
                  <p className="text-sm font-medium text-fg">Custom</p>
                  <p className="mt-0.5 text-xs text-fg-4">Manual server configuration</p>
                </div>
              </button>
            </div>
          )}
      ...
      {formMode === "preset" && selectedTool && (
        <form onSubmit={(e) => void handlePresetSubmit(e)} className="rounded-xl border border-edge bg-surface/50 p-5">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-fg-2">
              <span className="material-symbols-outlined text-accent text-base">
          ...
          {selectedTool.mcp_transport === "http" ? (
            <div className="mb-4">
              <label className="mb-1.5 flex items-center gap-1.5 text-xs font-medium text-fg-3">
                <span className="material-symbols-outlined text-sm text-cyan-400">cloud</span>
                Server URL
                <span className={`ml-auto rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider border-cyan-500/30 bg-cyan-500/10 text-cyan-400`}>
                  http
                </span>
              </label>
              <input
            ...
            <div className="mb-4 flex items-center gap-2 rounded-lg border border-edge bg-surface px-3 py-2">
              <span className="material-symbols-outlined text-sm text-purple-400">terminal</span>
          ...
          {selectedTool.required_config && selectedTool.required_config.length > 0 ? (
            <div className="space-y-3">
              {selectedTool.required_config.map((field) => (
                <div key={field.name}>
                  <label className="mb-1.5 flex items-center gap-1.5 text-xs font-medium text-fg-3">
                    {field.sensitive && <span className="material-symbols-outlined text-sm">key</span>}
                    {field.name}
                    <span className="text-fg-4 font-normal">— {field.description}</span>
                  </label>
                  <input
                    type={field.sensitive ? "password" : "text"}
                    value={presetEnv[field.name] ?? ""}
                    onChange={(e) => setPresetEnv((prev) => ({ ...prev, [field.name]: e.target.value }))}
            ...
            <p className="py-4 text-center text-sm text-fg-4">No configuration needed</p>
          )}
      ...
      {/* ── Custom Form ── */}
      {formMode === "custom" && (
        <form onSubmit={(e) => void handleCustomSubmit(e)} className="rounded-xl border border-edge bg-surface/50 p-5">
          <div className="mb-4 flex items-center justify-between">
            <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-fg-2">
              <span className="material-symbols-outlined text-accent text-base">add_circle</span>
              Add MCP Server
          ...
          <div className="mb-4 flex gap-1 rounded-lg border border-edge bg-surface p-1 w-fit">
            {(["stdio", "http"] as const).map((t) => (
              <button
                key={t}
                type="button"
                onClick={() => { setTransport(t); setValidationError(""); }}
...
function WorkspacesTab() {
  ...
  const totalMB = Math.round((workspaces?.reduce((sum, w) => sum + w.size_mb, 0) ?? 0) * 10) / 10;
